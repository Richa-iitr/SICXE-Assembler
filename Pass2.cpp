...
bool readIntermediateFile(ifstream& readFile,bool& isComment, int& lineNumber, int& address, int& blockNumber, string& label, string& opcode, string& operand, string& comment){
  ...
  if(isComment){
    firstNonWhiteSpace(fileLine,index,tempStatus,comment,true);
    return true;
  }
...
string createObjectCodeFormat34(){
  string objcode;
  int halfBytes;
  halfBytes = (getOperandFormat(opcode)=='+')?5:3;

  if(getOperandFormat(operand)=='#'){//Immediate
    ...
    if(ifAllNum(tempOperand)||((SYMTAB[tempOperand].exists=='y')&&(SYMTAB[tempOperand].relative==0))){//Immediate integer value
      int immediateValue;

      if(ifAllNum(tempOperand)){
  ...
  else if(getOperandFormat(operand)=='@'){
    string tempOperand = operand.substr(1,operand.length()-1);
  ...
  else if(getOperandFormat(operand)=='='){//Literals
    string tempOperand = operand.substr(1,operand.length()-1);
      ...
      if(OPTAB[getOpcode(opcode)].exists=='y'){
        if(OPTAB[getOpcode(opcode)].format==1){
          ...
          if(getOpcode(opcode)=="RSUB"){
            objectCode = OPTAB[getOpcode(opcode)].opcode;
            objectCode += (getOperandFormat(opcode)=='+')?"000000":"0000";
          }
          else{
